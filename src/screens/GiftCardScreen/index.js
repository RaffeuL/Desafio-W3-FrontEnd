...
export default function GiftCardScreen() {
  ...
  const [stores, setStores] = useState(['Americanas', 'Submarino', 'Shoptime', 'Extra', 'Ponto Frio'])
  ...
  function transfer() {
    if(agency === '1234' && account === '12345678'){
      Alert.alert('Success', `Gift Card of $${amount} successfully purchased  in ${selectedStore} store`)
    }else {
  ...
  return <>
    <View style={styles.screen}>
      ...
      <Input placeholder={'Amount'} onChangeText={setAmout} keyboardType={'numeric'} error={errors.amount} onFocus={() => {handleError(null, 'amount')}}/>
      <Picker 
      selectedValue={selectedStore}
      onValueChange={(itemValue, itemIndex) => setSelectedStore(itemValue)}
      style={styles.picker}
      onFocus={() => {handleError(null, 'store')}}
      >
        <Picker.Item label={'Select a store'} value={null} />
        {stores.map(store => { return <Picker.Item label={store} value={store} key={store}/>}) }
      </Picker>
       {errors.store && <Text style={{color: 'red', alignSelf: 'center'}}>{errors.store}</Text>}
      <Button label={'Transfer'} onPress={validate}></Button>
    </View>
  </>;
}
